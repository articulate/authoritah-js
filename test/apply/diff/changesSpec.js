...
const local = [
  { name: '1234', attr: 2, bobby: "tables", fake: { once: 1 }, empty: [], hashpty: {} },
  { name: '999', attr: 1, same: "tbh" },
  { name: '77910', attr: 2 },
];

const server = [
  { name: '1234', attr: 1, bubble: "bath", fake: { twice: 2 }, empty: [1,2,3], hashpty: { one: 'once' }, dingo: { one: 2 } },
...
describe("diffing for changes", () => {
  ...
  it('merges details from both', () => {
    expect(result[0]).to.have.all.keys('name', 'attr', 'bubble', 'bobby', 'fake', 'empty', 'hashpty', 'dingo');
  });
