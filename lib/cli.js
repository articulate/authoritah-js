...
function addDumpCommand(cli) {
  cli.command('dump [filename]').option('-s, --scripts <./rules>', "Folder path to save the rule scripts", "./rules").option('-d, --domain <domain>', "Auth0 domain to run against").option('-k, --key <key>', "The Auth0 domain key.").option('-s, --secret <secret>', "The Auth0 domain secret.").option('-F --format <format>', "Format to output rules.", "yaml").action(_index6.default);

  return cli;
}

function addApplyCommand(cli) {
  cli.command('apply [rules file]').option('-d, --domain <domain>', "Auth0 domain to run against").option('-k, --key <key>', "The Auth0 domain key.").option('-s, --secret <secret>', "The Auth0 domain secret.").action(_index8.default);
