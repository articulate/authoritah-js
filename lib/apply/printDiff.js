...
var printGroup = _ramda2.default.curry(function (printers, typeDef) {
  ...
  _ramda2.default.map(function (key) {
    ...
    _ramda2.default.map(function (actionName) {
      var diffs = fromObj(actions, actionName);
      var printer = printers(actionName);

      _ramda2.default.map(_ramda2.default.compose(printer, formatJSON), diffs);
    }, verbs);
  }, types);
...
function printDiff(context) {
  ...
  var removes = _context$say.warn;

  var types = _ramda2.default.keys(diff);
  var printers = fromObj({ adds: adds, changes: changes, removes: removes });

  _ramda2.default.map(_ramda2.default.compose(printGroup(printers), _ramda2.default.flip(_ramda2.default.pick)(diff), _ramda2.default.of), types);
