...
var configActions = {
  set: function set(args, context) {
    ...
    args.forEach(function (setting) {
      var _setting$split = setting.split('='),
          ...
          key = _setting$split2[0],
          val = _setting$split2[1];
  ...
  rm: function rm(args, context) {
    var config = context.config;

    config.remove(args);
  },
  ls: function ls(_args, context) {
    var config = context.config;
...
function index(command, args, options) {
  ...
  return (0, _loadEnv2.default)(options).then(_ramda2.default.partial(action, [args])).catch(error);
}