...
function index(filePath, options) {
  ...
  return (0, _loadEnv2.default)(options).then(_setupClient2.default).then(_fetchRules2.default).then(_fetchConnections2.default).then(_fetchClients2.default).then(_saveRules2.default).then(_saveConnections2.default).then(_associateClientConnections.associate).then((0, _saveManifest2.default)(filename)).then(ok('Manifest written to ' + filename)).catch(error);
}
