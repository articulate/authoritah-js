...
var printResult = function printResult(action, type, printer) {
  return function (_ref) {
    var name = _ref.name;
    return printer(_inflect2.default.capitalize(pastTense(action)) + ' ' + _inflect2.default.singularize(type) + ': ' + name);
  };
};
...
function apiCallWrapper(fnPath, context) {
  ...
  var apiFn = _ramda2.default.path([type, action], client);

  var printer = { delete: error, create: ok, update: notice }[action];
  var runApiAction = actions[action];

  return function (obj) {
    return _ramda2.default.equals(true, dryRun) ? printDryRun(action, type, printer, obj) : runApiAction(apiFn, obj).then(printResult(action, type, printer)).catch(function (err) {
